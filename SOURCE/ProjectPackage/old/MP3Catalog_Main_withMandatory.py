...
MANDATORY_SONGS         = '05 Canzoni Mandatory Copiate'        # INT  -
...
fldMANDATORY, fldSTART_ATTR, attribSONGSIZE, attribPUNTEGGIO = None, None, None, None
# songAttrName    = []
# baseAttribValue = []
...
MANDATORY_PRESET            = False         # indica se dobbimo copiare i mandatory prima degli altri
...
def prepareExcelHeader(rowValue=None):
    ...
    global fldMANDATORY, fldSTART_ATTR, attribSONGSIZE, attribPUNTEGGIO
    ...
    fldMANDATORY    = fldName.MANDATORY
...
def Mp3Extract():
    ...
    try:
        ...
        GoodAttributes  = extractSectID['Good Attributes']
        BadAttributes   = extractSectID['Bad Attributes']
    ...
    MyLogger.debug("Good Attributes:   %s" % (GoodAttributes))
    MyLogger.debug("Bad  Attributes:   %s" % (BadAttributes))
    ...
    for attrib in GoodAttributes:
        if attrib.upper() == 'MANDATORY':
            MANDATORY_PRESET = True
        MyLogger.info("Good     [%s]" % (attrib))
        try:
            i = songAttrName.index(attrib)
            MyLogger.debug(i)
            attribToExtract.append(i)
        except:
            MyLogger.debug("")
            pass

    for attrib in BadAttributes:
        MyLogger.info ("Bad      [%s]" % (attrib))
        try:
            i = songAttrName.index(attrib)
            MyLogger.debug(i)
            attribToAvoid.append(i)
        except:
            MyLogger.debug("")
            pass

    MyLogger.info("Good to extract:   %s" % (attribToExtract))
    MyLogger.info("Bad to avoid:      %s" % (attribToAvoid))
...
# ===============================================
def Songs_Analyze(workingTYPES, RandomDict, outFileList, fMANDATORY=False, fPUNTEGGIO=12345):
    fDEBUG = False
    MyLogger.warning('*'*40)
    if fMANDATORY:
        MyLogger.warning('Extracting MANDATORY songs')
    elif not fPUNTEGGIO == 12345:
    ...
    while workingTYPES:                                         # finchè abbiamo un TYPE
        for typeName in workingTYPES:                           # selezioniamo il TYPE
            ...
            songIndex = Songs_GetNext(RandomDict, typeName, fMANDATORY, fPUNTEGGIO)
            if songIndex < 0:
...
# ==============================================================================================================
# - Look for SONG with MANDATORY attribute
...
# -     TypeSectID[MANDATORY_SONGS]     numero di Mandatory copiate. Usato per il conteggio totale
# -     TypeSectID[NEXT_SONG_POINTER]   pointer allo TypeSectID[SHUFFLED_LIST] dell'ultima canzone cercata
# ==============================================================================================================
def Songs_GetNext(dict, typeName, fMANDATORY=False, fPUNTEGGIO=12345):
    ...
    while LOOP == True:                                         # finché è minore del numero di canzoni
        ...
        if songIndex >= TypeSectID[TOTAL_SONGS]:                   # finché è minore del numero di canzoni (parte da 0)
            if fMANDATORY:
                MyLogger.warning("")
                MyLogger.warning("[%s] - NO More Mandatory Songs available" % (typeName) )
                MyLogger.warning("")
            else:
                MyLogger.warning("")
        ...
        if fMANDATORY == True:
            attrib  = SongPTR[fldMANDATORY]                     # get Song Attrib
            if attrib.upper() == 'X':                           # and check if MANDATORY
                MyLogger.info("Found Mandatory: %s" % ( SongPTR) )
                return songIndex
            else:
                continue
        elif not fPUNTEGGIO == 12345:
...
def RandomExtract(mp3Dict):
    ...
    try:
        ...
        MAX_AUTHORS_SONGS         = RndSectID[STATUS_MAX_AUTHORS_SONGS]
    ...
    MyLogger.debug("MAX_AUTHORS_SONGS    : %s" % (MAX_AUTHORS_SONGS))
    ...
    for key, val in PercentDictID.items():
        ...
        TypeSectID[MANDATORY_SONGS]     = 0         # numero delle mandatory songs copiate
    ...
    debugListDisplay(RandomDict)
        ...
        # ===========================================================================
        # = Leggiamo tutti i Mandatory
        # - passiamo in rassegna l'intera lista delle canzoni
        # - alla ricerca delle mandatory e le copiamo nella destinazione.
        # - SWe invece usiamo il punteggio allora partiamo dal punteggio + alto.
        # ===========================================================================
    if globalARGs[EXTRACTION_TYPE] == 'Punteggioxx':
        ...
        workingTYPES = TYPES[:]     # copia dei TYPES cu cui andiamo a lavorare per non toccare l'originale utile per il Display
        if MANDATORY_PRESET:
            Songs_Analyze(workingTYPES, RandomDict, outFileList, fMANDATORY=True)
            if workingTYPES == []:
                MyLogger.info("No more TYPES available...")

        debugListDisplay(RandomDict)

        # ###################################
        choice = LnSys.getKeyboardInput("--------- Mandatory songs have been copied -------", keyLIST='ENTER', exitKey='QX', AnswerForDEBUG=None)
        # ###################################
...
def CopySongToDest(dict, typeName, songIndex, outFileList):
    ...
    if  TotalCopiedBytes+songSize > StatusSectID[STATUS_MAX_BYTES]:
        ...
        return (RCODE_MAX_AUTHOR_SONG_NUMBER, "[%s:%d] - Max song number [%d] per %s has been reached" % (typeName.upper(), TypeSectID[NEXT_SONG_POINTER], StatusSectID[STATUS_MAX_AUTHORS_SONGS], authorName))
...
# - Se retType == LIST allora verr?itornala una lista invece del DICT
# =======================================================================================
def extractSelected(dict, Punteggi=[], attribToExtract=[], attribToAvoid=[]):
    ...
    # if Punteggi == []:
    if globalARGs[EXTRACTION_TYPE] == 'Punteggio':
        MyLogger.info("                     %s" % (Punteggi) )
    else:
        MyLogger.info("Extracting attributes:")
        for i in attribToExtract:
            MyLogger.info("                     %s" % (songAttrName[i]) )
        MyLogger.info("Skipping attributes:")
        for i in attribToAvoid:
            MyLogger.info("                     %s" % (songAttrName[i]) )
    ...
    for typeName in dict.keys():
        ...
        for authorName in authorDict.keys():
            ...
            for albumName in albumDict.keys():
                # if albumName.upper().find('CONCERTO')   >= 0: continue
                # if albumName.upper().find('LIVE')       >= 0: continue
                songDict = albumDict.get(albumName)
                
                for songName, val in songDict.items():

                        # - Se ha un attributo NON voluto, ignoriamola
                    # if Punteggi != []:
                    if globalARGs[EXTRACTION_TYPE] == 'Punteggio':
                        ...
                        for i in attribToAvoid:
                            if val[i] == 'x':
                                break       # esci alla prima occorrenza

                            # - Se ha un attributo voluto, aggiungiamola
                            # cercare la colonna dei Punteggi
                        for i in attribToExtract:
                            if val[i] == 'x':
                                    # --------------------------------------
                                    # - puntamento/creazione del DICTionary
                                    # --------------------------------------
                                currAlbumPtr = outDict.getSectionPointer(typeName+';'+authorName+';'+albumName, fldSep=';', create=True)
                                currAlbumPtr[songName] = val
                                break       # esci alla prima occorrenza

...
# ####################################################################################################################
# https://raw.githubusercontent.com/senko/python-LnLogger/master/LnLogger.py
...
class LnLogger(object):
    """
    LnLogger mixin/base class adding verbose logging to subclasses.
    ...
    without any timestamping. A static method LnLogger.basicConfig() is provided
    ...
    >>> LnLogger.basicConfig()

    Example mixin usage:

    >>> class MyClass(LnLogger):
    ...    def my_method(self):
    ...        self.debug('called')
    ...    def raises_exc(self):
    ...        try:
    ...
    Module also provides a singleton "LnLogger" instance of LnLogger class, which
    can be used when it's not feasible to use the mixin. The LnLogger provides
    the same debug(), warning() and error() methods.

    Example singleton usage:

    >>> LnLogger.debug('This is a debug message')
    """
    ...
    def _raw_log(self, logfn, message, exc_info):
        ...
        if len(tb) > 2:
            ...
            fn = tb[-3][2]
            if fn != '<module>':
                if self.__class__.__name__ != LnLogger.__name__:
